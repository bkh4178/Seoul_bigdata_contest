...
# S3: score_df 전체에서 기존 센터 행정동만 제외
...
# ══════════════════════════════════════════════════════════
# 5. 새로 커버한 공백지역 취약도 비교
#    "어떤 시나리오가 더 취약한 공백지역을 커버했는가"
# ══════════════════════════════════════════════════════════
log(f"\n[4] 새로 커버한 공백지역 취약도 비교")

# 공백지역 행정동의 Final_Score_4
gap_scores = gap_df["Final_Score_4"].values

def calc_newly_covered_score(new_centers):
    """
    신규 센터 추가 후 새로 2km 이내로 커버된 공백지역의 평균 Final_Score_4
    (기존 S0에서 2km 초과였던 행정동 중, 신규 배치 후 2km 이내가 된 것)
    """
    all_centers = np.vstack([EXISTING_CENTERS, new_centers])
    tree = cKDTree(all_centers)
    new_dists, _ = tree.query(GAP_COORDS)
    # 새로 커버 = 신규 배치 후 2km 이내
    newly_covered = new_dists <= 2000
    if newly_covered.sum() == 0:
        return np.nan, 0
    avg_score = gap_scores[newly_covered].mean()
    count = newly_covered.sum()
    return avg_score, count

# S1 랜덤 1,000회 — 새로 커버한 공백지역 취약도 분포
log(f"  S1 랜덤 {N_RANDOM}회 반복 중...")
rng = np.random.default_rng(RANDOM_SEED)
pool_coords = pool_df[["rep_x","rep_y"]].values
pool_idx    = np.arange(len(pool_coords))

random_covered_scores = []
random_covered_counts = []
for i in range(N_RANDOM):
    sampled_idx = rng.choice(pool_idx, size=N_NEW, replace=False)
    sampled_centers = pool_coords[sampled_idx]
    avg_s, cnt = calc_newly_covered_score(sampled_centers)
    random_covered_scores.append(avg_s)
    random_covered_counts.append(cnt)
    if (i+1) % 200 == 0:
        log(f"  {i+1}/{N_RANDOM} 완료...")

random_covered_scores = np.array(random_covered_scores)
log(f"  S1 새 커버 공백지역 취약도: 평균={np.nanmean(random_covered_scores):.2f}  "
    f"std={np.nanstd(random_covered_scores):.2f}")

# S2 / S3 / S4
s2_cov_score, s2_cov_cnt = calc_newly_covered_score(S2_CENTERS)
s3_cov_score, s3_cov_cnt = calc_newly_covered_score(S3_CENTERS)
s4_cov_score, s4_cov_cnt = calc_newly_covered_score(S4_CENTERS) if len(S4_CENTERS) == N_NEW else (np.nan, 0)
s1_mean_score = np.nanmean(random_covered_scores)

log(f"\n  [새로 커버한 공백지역 평균 취약도 (Final_Score_4)]")
log(f"  S1 랜덤 평균 : {s1_mean_score:.2f}  (커버 수 평균: {np.mean(random_covered_counts):.1f}개)")
log(f"  S2 수요기준  : {s2_cov_score:.2f}  (커버 수: {s2_cov_cnt}개)")
log(f"  S3 MCI단독   : {s3_cov_score:.2f}  (커버 수: {s3_cov_cnt}개)  ※ 기존 센터 중복 무시")
log(f"  S4 최종제안  : {s4_cov_score:.2f}  (커버 수: {s4_cov_cnt}개)")

# S4 백분위 (취약도 기준 — 높을수록 좋음)
if not np.isnan(s4_cov_score):
    pct_score = (random_covered_scores < s4_cov_score).mean() * 100
    log(f"\n  ★ S4는 랜덤 분포 대비 상위 {100-pct_score:.1f}% 취약도 지역을 커버")
    log(f"    (랜덤 {N_RANDOM}회 중 {pct_score:.1f}%보다 더 취약한 공백지역 커버)")
...
# 취약도 비교 테이블
coverage_results = [
    {"시나리오": "S1(랜덤평균)", "새커버공백지역수": round(np.mean(random_covered_counts),1),
     "평균취약도(Final_Score_4)": round(s1_mean_score, 2), "비고": "1000회 평균"},
    {"시나리오": "S2(수요기준)", "새커버공백지역수": s2_cov_cnt,
     "평균취약도(Final_Score_4)": round(s2_cov_score, 2), "비고": ""},
    {"시나리오": "S3(MCI단독)", "새커버공백지역수": s3_cov_cnt,
     "평균취약도(Final_Score_4)": round(s3_cov_score, 2), "비고": "기존센터 중복 무시"},
    {"시나리오": "S4(최종제안)", "새커버공백지역수": s4_cov_cnt,
     "평균취약도(Final_Score_4)": round(s4_cov_score, 2), "비고": ""},
]
cov_df = pd.DataFrame(coverage_results)
cov_df.to_csv(OUT / "coverage_score_result.csv", index=False, encoding="utf-8-sig")
log(f"\n  coverage_score_result.csv 저장")
...
ax.hist(random_covered_scores, bins=50, color="#4A90D9", alpha=0.7,
        edgecolor="white", label=f"랜덤 배치 {N_RANDOM}회")
if not np.isnan(s4_cov_score):
    ax.axvline(s4_cov_score, color="#E74C3C", linewidth=2.5,
               label=f"S4 최종제안 ({s4_cov_score:.2f}점)")
ax.axvline(s1_mean_score, color="#888", linewidth=1.5,
           linestyle="--", label=f"S1 평균 ({s1_mean_score:.2f}점)")
ax.set_xlabel("새로 커버한 공백지역 평균 취약도 (Final_Score_4)")
ax.set_ylabel("빈도")
